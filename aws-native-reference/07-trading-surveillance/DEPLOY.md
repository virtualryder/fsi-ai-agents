# Deploy — Trading Surveillance (`07-trading-surveillance`)
...
docker buildx build --platform linux/arm64 \
  -f aws-native-reference/07-trading-surveillance/Dockerfile -t $ECR:07-trading-surveillance --push .
```
...
agentcore configure --entrypoint serve:app --container $ECR:07-trading-surveillance --name fsi-07-trading-surveillance
agentcore launch                 # creates the AgentCore Runtime from the ARM64 image
agentcore invoke '{"alert_id": "SURV-1", "alert_type": "LAYERING", "trader_id": "TR-1"}'      # test /invocations through the runtime
```
...
terraform init
terraform apply \
  -var="agent_key=07-trading-surveillance" \
  -var="image_uri=$ECR:07-trading-surveillance" \
  -var='vpc_subnet_ids=["subnet-aaa","subnet-bbb"]' \
  -var="audit_table_arn=arn:aws:dynamodb:$AWS_REGION:$ACCOUNT:table/07-trading-surveillance" \
...
docker run --rm -p 8080:8080 -e AGENT=07-trading-surveillance -e LLM_PROVIDER=anthropic $ECR:07-trading-surveillance &
curl localhost:8080/ping
curl -XPOST localhost:8080/invocations -H 'content-type: application/json' \
  -d '{"input": {"alert_id": "SURV-1", "alert_type": "LAYERING", "trader_id": "TR-1"}}'
```

`/ping` returns `{"status":"Healthy"}`. `/invocations` runs the agent graph and
returns `{"agent":"07-trading-surveillance","status":"OK","output":{...}}`, where `output`
...
- The sample input above is illustrative; the authoritative input shape is this
  agent's `agent/state.py` and `app.py` in [`../../07-trading-surveillance-agent`](../../07-trading-surveillance-agent).
