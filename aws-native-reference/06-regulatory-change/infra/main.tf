# AWS-native Regulatory Change Management — infrastructure.
...
locals {
  name = "fsi-regchange-${var.environment}"
  lambdas = {
    intake                 = "lambdas.intake.handler"
    impact                 = "lambdas.impact.handler"
    gap_analysis           = "lambdas.gap_analysis.handler"
    cco_notify             = "lambdas.cco_notify.handler"
...
resource "aws_sfn_state_machine" "regchange" {
  name     = local.name
  role_arn = aws_iam_role.sfn.arn
  definition = templatefile("${path.module}/../stepfunctions/regulatory_change.asl.json", {
    IntakeFunctionArn                = aws_lambda_function.node["intake"].arn
    ImpactFunctionArn                = aws_lambda_function.node["impact"].arn
    GapAnalysisFunctionArn           = aws_lambda_function.node["gap_analysis"].arn
    CcoNotifyFunctionArn             = aws_lambda_function.node["cco_notify"].arn
