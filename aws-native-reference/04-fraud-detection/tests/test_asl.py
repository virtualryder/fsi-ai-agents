import json, os
ASL = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "stepfunctions", "fraud_detection.asl.json")
...
def test_hitl_waitfortasktoken():
    g=_load()["States"]["AnalystReviewGate"]
    assert g["Resource"].endswith(".waitForTaskToken") and g["Parameters"]["Payload"]["task_token.$"]=="$$.Task.Token"
def test_choice_default_allow(): assert _load()["States"]["DispositionChoice"]["Default"]=="Allow"
