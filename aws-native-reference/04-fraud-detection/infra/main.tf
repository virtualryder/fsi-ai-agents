# AWS-native Real-Time Fraud Detection — infrastructure.
...
locals {
  name = "fsi-fraud-${var.environment}"
  lambdas = {
    intake                 = "lambdas.intake.handler"
    score                  = "lambdas.score.handler"
    draft_reg_e            = "lambdas.draft_reg_e.handler"
    step_up                = "lambdas.step_up.handler"
    analyst_notify         = "lambdas.analyst_notify.handler"
    allow                  = "lambdas.allow.handler"
...
resource "aws_sfn_state_machine" "fraud" {
  name     = local.name
  role_arn = aws_iam_role.sfn.arn
  definition = templatefile("${path.module}/../stepfunctions/fraud_detection.asl.json", {
    IntakeFunctionArn                = aws_lambda_function.node["intake"].arn
    ScoreFunctionArn                 = aws_lambda_function.node["score"].arn
    DraftRegEFunctionArn             = aws_lambda_function.node["draft_reg_e"].arn
    StepUpFunctionArn                = aws_lambda_function.node["step_up"].arn
    AnalystNotifyFunctionArn         = aws_lambda_function.node["analyst_notify"].arn
    AllowFunctionArn                 = aws_lambda_function.node["allow"].arn
