# Deploy — KYC/CDD Perpetual Monitoring (`03-kyc-cdd-perpetual`)
...
docker buildx build --platform linux/arm64 \
  -f aws-native-reference/03-kyc-cdd-perpetual/Dockerfile -t $ECR:03-kyc-cdd-perpetual --push .
```
...
agentcore configure --entrypoint serve:app --container $ECR:03-kyc-cdd-perpetual --name fsi-03-kyc-cdd-perpetual
agentcore launch                 # creates the AgentCore Runtime from the ARM64 image
agentcore invoke '{"customer_id": "CUST-001", "trigger_type": "ADVERSE_MEDIA"}'      # test /invocations through the runtime
```
...
terraform init
terraform apply \
  -var="agent_key=03-kyc-cdd-perpetual" \
  -var="image_uri=$ECR:03-kyc-cdd-perpetual" \
  -var='vpc_subnet_ids=["subnet-aaa","subnet-bbb"]' \
  -var="audit_table_arn=arn:aws:dynamodb:$AWS_REGION:$ACCOUNT:table/03-kyc-cdd-perpetual" \
...
docker run --rm -p 8080:8080 -e AGENT=03-kyc-cdd-perpetual -e LLM_PROVIDER=anthropic $ECR:03-kyc-cdd-perpetual &
curl localhost:8080/ping
curl -XPOST localhost:8080/invocations -H 'content-type: application/json' \
  -d '{"input": {"customer_id": "CUST-001", "trigger_type": "ADVERSE_MEDIA"}}'
```

`/ping` returns `{"status":"Healthy"}`. `/invocations` runs the agent graph and
returns `{"agent":"03-kyc-cdd-perpetual","status":"OK","output":{...}}`, where `output`
...
- The sample input above is illustrative; the authoritative input shape is this
  agent's `agent/state.py` and `app.py` in [`../../03-kyc-cdd-perpetual-agent`](../../03-kyc-cdd-perpetual-agent).
