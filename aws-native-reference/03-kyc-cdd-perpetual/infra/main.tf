# AWS-native KYC/CDD Perpetual Monitoring — infrastructure.
...
locals {
  name = "fsi-kyc-${var.environment}"
  lambdas = {
    screen                 = "lambdas.screen.handler"
    rescore                = "lambdas.rescore.handler"
    draft_edd              = "lambdas.draft_edd.handler"
...
resource "aws_sfn_state_machine" "kyc" {
  name     = local.name
  role_arn = aws_iam_role.sfn.arn
  definition = templatefile("${path.module}/../stepfunctions/kyc_cdd_review.asl.json", {
    ScreenFunctionArn                = aws_lambda_function.node["screen"].arn
    RescoreFunctionArn               = aws_lambda_function.node["rescore"].arn
    DraftEddFunctionArn              = aws_lambda_function.node["draft_edd"].arn
