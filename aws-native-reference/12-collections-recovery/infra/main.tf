# AWS-native Collections & Recovery — infrastructure.
...
locals {
  name = "fsi-collections-${var.environment}"
  lambdas = {
    intake                 = "lambdas.intake.handler"
    assess                 = "lambdas.assess.handler"
    supervisor_notify      = "lambdas.supervisor_notify.handler"
    draft_letter           = "lambdas.draft_letter.handler"
...
resource "aws_sfn_state_machine" "collections" {
  name     = local.name
  role_arn = aws_iam_role.sfn.arn
  definition = templatefile("${path.module}/../stepfunctions/collections_recovery.asl.json", {
    IntakeFunctionArn                = aws_lambda_function.node["intake"].arn
    AssessFunctionArn                = aws_lambda_function.node["assess"].arn
    SupervisorNotifyFunctionArn      = aws_lambda_function.node["supervisor_notify"].arn
    DraftLetterFunctionArn           = aws_lambda_function.node["draft_letter"].arn
