# Deploy — Collections & Recovery (`12-collections-recovery`)
...
docker buildx build --platform linux/arm64 \
  -f aws-native-reference/12-collections-recovery/Dockerfile -t $ECR:12-collections-recovery --push .
```
...
agentcore configure --entrypoint serve:app --container $ECR:12-collections-recovery --name fsi-12-collections-recovery
agentcore launch                 # creates the AgentCore Runtime from the ARM64 image
agentcore invoke '{"account_id": "ACC-1", "consumer_timezone": "America/New_York"}'      # test /invocations through the runtime
```
...
terraform init
terraform apply \
  -var="agent_key=12-collections-recovery" \
  -var="image_uri=$ECR:12-collections-recovery" \
  -var='vpc_subnet_ids=["subnet-aaa","subnet-bbb"]' \
  -var="audit_table_arn=arn:aws:dynamodb:$AWS_REGION:$ACCOUNT:table/12-collections-recovery" \
...
docker run --rm -p 8080:8080 -e AGENT=12-collections-recovery -e LLM_PROVIDER=anthropic $ECR:12-collections-recovery &
curl localhost:8080/ping
curl -XPOST localhost:8080/invocations -H 'content-type: application/json' \
  -d '{"input": {"account_id": "ACC-1", "consumer_timezone": "America/New_York"}}'
```

`/ping` returns `{"status":"Healthy"}`. `/invocations` runs the agent graph and
returns `{"agent":"12-collections-recovery","status":"OK","output":{...}}`, where `output`
...
- The sample input above is illustrative; the authoritative input shape is this
  agent's `agent/state.py` and `app.py` in [`../../12-collections-recovery-agent`](../../12-collections-recovery-agent).
