import json, os
ASL = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "stepfunctions", "collections_recovery.asl.json")
...
def test_hitl_waitfortasktoken():
    g=_load()["States"]["SupervisorReviewGate"]
    assert g["Resource"].endswith(".waitForTaskToken") and g["Parameters"]["Payload"]["task_token.$"]=="$$.Task.Token"
def test_choice_default_draft_letter(): assert _load()["States"]["DispositionChoice"]["Default"]=="DraftLetter"
