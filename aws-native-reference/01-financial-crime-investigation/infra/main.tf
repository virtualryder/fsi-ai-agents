# AWS-native Financial Crime Investigation (SAR) — infrastructure.
...
locals {
  name = "fsi-fincrime-${var.environment}"
  lambdas = {
    screen       = "lambdas.screen.handler"
    score        = "lambdas.score.handler"
    route        = "lambdas.route.handler"
    generate_sar = "lambdas.generate_sar.handler"
    hitl_notify  = "lambdas.hitl_notify.handler"
    close_case   = "lambdas.close_case.handler"
...
resource "aws_sfn_state_machine" "fincrime" {
  name     = local.name
  role_arn = aws_iam_role.sfn.arn
  definition = templatefile("${path.module}/../stepfunctions/financial_crime_investigation.asl.json", {
    ScreenFunctionArn      = aws_lambda_function.node["screen"].arn
    ScoreFunctionArn       = aws_lambda_function.node["score"].arn
    RouteFunctionArn       = aws_lambda_function.node["route"].arn
    GenerateSarFunctionArn = aws_lambda_function.node["generate_sar"].arn
    HitlNotifyFunctionArn  = aws_lambda_function.node["hitl_notify"].arn
    CloseCaseFunctionArn   = aws_lambda_function.node["close_case"].arn
