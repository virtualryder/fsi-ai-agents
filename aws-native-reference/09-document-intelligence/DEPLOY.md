# Deploy — Document Intelligence (`09-document-intelligence`)
...
> **This agent also has a full native rebuild** in this folder (`core.py`, `strands_agent.py`, `stepfunctions/`, `lambdas/`, `infra/`) using **Strands + Step Functions** — see `README.md` and `DEPLOYMENT-RUNBOOK.md`. The container path below is the lift-and-shift alternative that runs the original LangGraph agent unchanged.
...
docker buildx build --platform linux/arm64 \
  -f aws-native-reference/09-document-intelligence/Dockerfile -t $ECR:09-document-intelligence --push .
```
...
agentcore configure --entrypoint serve:app --container $ECR:09-document-intelligence --name fsi-09-document-intelligence
agentcore launch                 # creates the AgentCore Runtime from the ARM64 image
agentcore invoke '{"document": {"doc_id": "D-1", "text": "FORM 1003 Uniform Residential Loan Application"}}'      # test /invocations through the runtime
```
...
terraform init
terraform apply \
  -var="agent_key=09-document-intelligence" \
  -var="image_uri=$ECR:09-document-intelligence" \
  -var='vpc_subnet_ids=["subnet-aaa","subnet-bbb"]' \
  -var="audit_table_arn=arn:aws:dynamodb:$AWS_REGION:$ACCOUNT:table/09-document-intelligence" \
...
docker run --rm -p 8080:8080 -e AGENT=09-document-intelligence -e LLM_PROVIDER=anthropic $ECR:09-document-intelligence &
curl localhost:8080/ping
curl -XPOST localhost:8080/invocations -H 'content-type: application/json' \
  -d '{"input": {"document": {"doc_id": "D-1", "text": "FORM 1003 Uniform Residential Loan Application"}}}'
```

`/ping` returns `{"status":"Healthy"}`. `/invocations` runs the agent graph and
returns `{"agent":"09-document-intelligence","status":"OK","output":{...}}`, where `output`
...
- The sample input above is illustrative; the authoritative input shape is this
  agent's `agent/state.py` and `app.py` in [`../../09-document-intelligence-agent`](../../09-document-intelligence-agent).
