# Deploy — Credit Underwriting (`08-credit-underwriting`)
...
docker buildx build --platform linux/arm64 \
  -f aws-native-reference/08-credit-underwriting/Dockerfile -t $ECR:08-credit-underwriting --push .
```
...
agentcore configure --entrypoint serve:app --container $ECR:08-credit-underwriting --name fsi-08-credit-underwriting
agentcore launch                 # creates the AgentCore Runtime from the ARM64 image
agentcore invoke '{"application_id": "APP-1", "loan_type": "CONVENTIONAL"}'      # test /invocations through the runtime
```
...
terraform init
terraform apply \
  -var="agent_key=08-credit-underwriting" \
  -var="image_uri=$ECR:08-credit-underwriting" \
  -var='vpc_subnet_ids=["subnet-aaa","subnet-bbb"]' \
  -var="audit_table_arn=arn:aws:dynamodb:$AWS_REGION:$ACCOUNT:table/08-credit-underwriting" \
...
docker run --rm -p 8080:8080 -e AGENT=08-credit-underwriting -e LLM_PROVIDER=anthropic $ECR:08-credit-underwriting &
curl localhost:8080/ping
curl -XPOST localhost:8080/invocations -H 'content-type: application/json' \
  -d '{"input": {"application_id": "APP-1", "loan_type": "CONVENTIONAL"}}'
```

`/ping` returns `{"status":"Healthy"}`. `/invocations` runs the agent graph and
returns `{"agent":"08-credit-underwriting","status":"OK","output":{...}}`, where `output`
...
- The sample input above is illustrative; the authoritative input shape is this
  agent's `agent/state.py` and `app.py` in [`../../08-credit-underwriting-agent`](../../08-credit-underwriting-agent).
