# AWS-native Payments Compliance — infrastructure.
...
locals {
  name = "fsi-payments-${var.environment}"
  lambdas = {
    ...
    auto_resolve = "lambdas.auto_resolve.handler"
    draft_notice = "lambdas.draft_notice.handler"
...
resource "aws_sfn_state_machine" "payments" {
  name     = local.name
  role_arn = aws_iam_role.sfn.arn
  definition = templatefile("${path.module}/../stepfunctions/payments_compliance.asl.json", {
    ...
    AutoResolveFunctionArn = aws_lambda_function.node["auto_resolve"].arn
    DraftNoticeFunctionArn = aws_lambda_function.node["draft_notice"].arn
