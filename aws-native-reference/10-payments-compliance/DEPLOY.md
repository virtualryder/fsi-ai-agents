# Deploy — Payments Compliance (`10-payments-compliance`)
...
docker buildx build --platform linux/arm64 \
  -f aws-native-reference/10-payments-compliance/Dockerfile -t $ECR:10-payments-compliance --push .
```
...
agentcore configure --entrypoint serve:app --container $ECR:10-payments-compliance --name fsi-10-payments-compliance
agentcore launch                 # creates the AgentCore Runtime from the ARM64 image
agentcore invoke '{"payment_event": {"payment_id": "P-1", "event_type": "ACH_RETURN", "return_code": "R10"}}'      # test /invocations through the runtime
```
...
terraform init
terraform apply \
  -var="agent_key=10-payments-compliance" \
  -var="image_uri=$ECR:10-payments-compliance" \
  -var='vpc_subnet_ids=["subnet-aaa","subnet-bbb"]' \
  -var="audit_table_arn=arn:aws:dynamodb:$AWS_REGION:$ACCOUNT:table/10-payments-compliance" \
...
docker run --rm -p 8080:8080 -e AGENT=10-payments-compliance -e LLM_PROVIDER=anthropic $ECR:10-payments-compliance &
curl localhost:8080/ping
curl -XPOST localhost:8080/invocations -H 'content-type: application/json' \
  -d '{"input": {"payment_event": {"payment_id": "P-1", "event_type": "ACH_RETURN", "return_code": "R10"}}}'
```

`/ping` returns `{"status":"Healthy"}`. `/invocations` runs the agent graph and
returns `{"agent":"10-payments-compliance","status":"OK","output":{...}}`, where `output`
...
- The sample input above is illustrative; the authoritative input shape is this
  agent's `agent/state.py` and `app.py` in [`../../10-payments-compliance-agent`](../../10-payments-compliance-agent).
