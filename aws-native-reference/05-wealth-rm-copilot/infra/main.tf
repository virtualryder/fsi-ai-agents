# AWS-native Wealth & RM Copilot — infrastructure.
...
locals {
  name = "fsi-wealth-${var.environment}"
  lambdas = {
    intake                 = "lambdas.intake.handler"
    suitability            = "lambdas.suitability.handler"
    block_unsuitable       = "lambdas.block_unsuitable.handler"
    recommend              = "lambdas.recommend.handler"
    rm_notify              = "lambdas.rm_notify.handler"
...
resource "aws_sfn_state_machine" "wealth" {
  name     = local.name
  role_arn = aws_iam_role.sfn.arn
  definition = templatefile("${path.module}/../stepfunctions/wealth_rm.asl.json", {
    IntakeFunctionArn                = aws_lambda_function.node["intake"].arn
    SuitabilityFunctionArn           = aws_lambda_function.node["suitability"].arn
    BlockUnsuitableFunctionArn       = aws_lambda_function.node["block_unsuitable"].arn
    RecommendFunctionArn             = aws_lambda_function.node["recommend"].arn
    RmNotifyFunctionArn              = aws_lambda_function.node["rm_notify"].arn
