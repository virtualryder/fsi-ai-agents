# Deploy — Wealth & RM Copilot (`05-wealth-rm-copilot`)
...
docker buildx build --platform linux/arm64 \
  -f aws-native-reference/05-wealth-rm-copilot/Dockerfile -t $ECR:05-wealth-rm-copilot --push .
```
...
agentcore configure --entrypoint serve:app --container $ECR:05-wealth-rm-copilot --name fsi-05-wealth-rm-copilot
agentcore launch                 # creates the AgentCore Runtime from the ARM64 image
agentcore invoke '{"request_id": "REQ-1", "client_id": "CL-1", "request_type": "MEETING_PREP"}'      # test /invocations through the runtime
```
...
terraform init
terraform apply \
  -var="agent_key=05-wealth-rm-copilot" \
  -var="image_uri=$ECR:05-wealth-rm-copilot" \
  -var='vpc_subnet_ids=["subnet-aaa","subnet-bbb"]' \
  -var="audit_table_arn=arn:aws:dynamodb:$AWS_REGION:$ACCOUNT:table/05-wealth-rm-copilot" \
...
docker run --rm -p 8080:8080 -e AGENT=05-wealth-rm-copilot -e LLM_PROVIDER=anthropic $ECR:05-wealth-rm-copilot &
curl localhost:8080/ping
curl -XPOST localhost:8080/invocations -H 'content-type: application/json' \
  -d '{"input": {"request_id": "REQ-1", "client_id": "CL-1", "request_type": "MEETING_PREP"}}'
```

`/ping` returns `{"status":"Healthy"}`. `/invocations` runs the agent graph and
returns `{"agent":"05-wealth-rm-copilot","status":"OK","output":{...}}`, where `output`
...
- The sample input above is illustrative; the authoritative input shape is this
  agent's `agent/state.py` and `app.py` in [`../../05-wealth-rm-copilot`](../../05-wealth-rm-copilot).
