import json, os
ASL = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "stepfunctions", "wealth_rm.asl.json")
...
def test_hitl_waitfortasktoken():
    g=_load()["States"]["RMApprovalGate"]
    assert g["Resource"].endswith(".waitForTaskToken") and g["Parameters"]["Payload"]["task_token.$"]=="$$.Task.Token"
def test_unsuitable_default_recommend():
    assert _load()["States"]["SuitabilityChoice"]["Default"]=="Recommend"
