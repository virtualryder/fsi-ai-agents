# Deploy — AML/TMS False-Positive Triage (`02-aml-tms-enhancement`)
...
> **This agent also has a full native rebuild** in this folder (`core.py`, `strands_agent.py`, `stepfunctions/`, `lambdas/`, `infra/`, `tests/`) using **Strands + Step Functions** with a `waitForTaskToken` HITL gate — see [`README.md`](./README.md). This container path is the lift-and-shift alternative that runs the original LangGraph agent unchanged.
...
docker buildx build --platform linux/arm64 \
  -f aws-native-reference/02-aml-tms-enhancement/Dockerfile -t $ECR:02-aml-tms-enhancement --push .
```
...
agentcore configure --entrypoint serve:app --container $ECR:02-aml-tms-enhancement --name fsi-02-aml-tms-enhancement
agentcore launch                 # creates the AgentCore Runtime from the ARM64 image
agentcore invoke '{"raw_alert": {"alert_id": "A-1", "alert_type": "VELOCITY", "amount": 9500, "customer_id": "CUST-001", "triggered_rule": "CASH-SUB-10K"}}'      # test /invocations through the runtime
```
...
terraform init
terraform apply \
  -var="agent_key=02-aml-tms-enhancement" \
  -var="image_uri=$ECR:02-aml-tms-enhancement" \
  -var='vpc_subnet_ids=["subnet-aaa","subnet-bbb"]' \
  -var="audit_table_arn=arn:aws:dynamodb:$AWS_REGION:$ACCOUNT:table/02-aml-tms-enhancement" \
...
docker run --rm -p 8080:8080 -e AGENT=02-aml-tms-enhancement -e LLM_PROVIDER=anthropic $ECR:02-aml-tms-enhancement &
curl localhost:8080/ping
curl -XPOST localhost:8080/invocations -H 'content-type: application/json' \
  -d '{"input": {"raw_alert": {"alert_id": "A-1", "alert_type": "VELOCITY", "amount": 9500, "customer_id": "CUST-001", "triggered_rule": "CASH-SUB-10K"}}}'
```

`/ping` returns `{"status":"Healthy"}`. `/invocations` runs the agent graph and
returns `{"agent":"02-aml-tms-enhancement","status":"OK","output":{...}}`, where `output`
...
- The sample input above is illustrative; the authoritative input shape is this
  agent's `agent/state.py` and `app.py` in [`../../02-aml-tms-enhancement-agent`](../../02-aml-tms-enhancement-agent).
