# AWS-native Model Risk Management (SR 11-7) — infrastructure.
...
locals {
  name = "fsi-modelrisk-${var.environment}"
  lambdas = {
    inventory              = "lambdas.inventory.handler"
    assess                 = "lambdas.assess.handler"
    narrative              = "lambdas.narrative.handler"
    mro_notify             = "lambdas.mro_notify.handler"
...
resource "aws_sfn_state_machine" "modelrisk" {
  name     = local.name
  role_arn = aws_iam_role.sfn.arn
  definition = templatefile("${path.module}/../stepfunctions/model_risk.asl.json", {
    InventoryFunctionArn             = aws_lambda_function.node["inventory"].arn
    AssessFunctionArn                = aws_lambda_function.node["assess"].arn
    NarrativeFunctionArn             = aws_lambda_function.node["narrative"].arn
    MroNotifyFunctionArn             = aws_lambda_function.node["mro_notify"].arn
