# Deploy — Model Risk Management (`11-model-risk`)
...
docker buildx build --platform linux/arm64 \
  -f aws-native-reference/11-model-risk/Dockerfile -t $ECR:11-model-risk --push .
```
...
agentcore configure --entrypoint serve:app --container $ECR:11-model-risk --name fsi-11-model-risk
agentcore launch                 # creates the AgentCore Runtime from the ARM64 image
agentcore invoke '{"model_id": "AGT08-CREDIT-SCORE-v1", "validation_type": "ANNUAL"}'      # test /invocations through the runtime
```
...
terraform init
terraform apply \
  -var="agent_key=11-model-risk" \
  -var="image_uri=$ECR:11-model-risk" \
  -var='vpc_subnet_ids=["subnet-aaa","subnet-bbb"]' \
  -var="audit_table_arn=arn:aws:dynamodb:$AWS_REGION:$ACCOUNT:table/11-model-risk" \
...
docker run --rm -p 8080:8080 -e AGENT=11-model-risk -e LLM_PROVIDER=anthropic $ECR:11-model-risk &
curl localhost:8080/ping
curl -XPOST localhost:8080/invocations -H 'content-type: application/json' \
  -d '{"input": {"model_id": "AGT08-CREDIT-SCORE-v1", "validation_type": "ANNUAL"}}'
```

`/ping` returns `{"status":"Healthy"}`. `/invocations` runs the agent graph and
returns `{"agent":"11-model-risk","status":"OK","output":{...}}`, where `output`
...
- The sample input above is illustrative; the authoritative input shape is this
  agent's `agent/state.py` and `app.py` in [`../../11-model-risk-agent`](../../11-model-risk-agent).
