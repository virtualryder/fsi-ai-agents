"""
MCP authorization gateway (Phase 3) — the governed front door to systems of record.
...
Every agent tool call passes through one enforcement point that, in order:
  ...
  3. For HIGH-RISK (write/irreversible) tools, requires HUMAN APPROVAL before
     execution (tool-call approval gate); a verified reviewer identity is bound
     into the record.
  4. MINTS a short-lived token scoped to exactly that tool, carrying user context
     (no standing service account).
  5. INVOKES the tool via the Phase 2 connector framework.
...
This is the reference logic for what AWS deploys as Bedrock AgentCore Gateway +
AgentCore Identity (or API Gateway + Lambda authorizer + STS + Cognito + OPA/
Cedar). The decision/least-privilege/token/audit semantics are identical.
"""
...
_ROLE_CLAIM = os.getenv("AUTH_ROLE_CLAIM", "custom:bsa_role")


def _roles_from_claims(claims: Dict[str, Any]) -> List[str]:
    """Extract roles from common claim shapes (string, list, or cognito:groups)."""
...
class MCPGateway:
    ...
    def invoke(
        ...
        *,
        ...
        # 5. Invoke the tool via the Phase 2 connector framework
        try:
